...
public class Upload_Controller {
	...
	//개발자 장윤용
		...
		@RequestMapping(value = "/postAnnounce", method = RequestMethod.POST)
		  @ResponseBody
		  public String postAnnounce( Model model, MultipartHttpServletRequest request,
			...
			Bean getNo = (Bean)Repository.getleaderWriteNo(memno);
			if(getNo == null){
				bean.setNo(1);
			}else{
				no = getNo.getNo();
			...
			Repository.insertleaderWrite(bean);
			...
			Iterator<String> itr =  request.getFileNames();
		      ...
		      while(itr.hasNext()) {
		          ...
		          String updata_path = "file_"+bean.getLeadno();
		          File uploadDir = new File("/Users/hongdavid/git/Bit_Place/src/main/webapp/files/"+updata_path);
		          uploadDir.mkdir();
		          File uploadFile = new File(uploadDir + "/" + mpf.getOriginalFilename());
		         
		          mpf.transferTo(uploadFile);
		          
		          bean.setUpdata(mpf.getOriginalFilename());
		          bean.setUpdata_path(updata_path);
		          
		          Repository.insertData(bean);
