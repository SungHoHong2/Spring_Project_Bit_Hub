...
$(document).ready(function(){
  $(".glyphicon-tooltip").mouseover(function(){
   ...
   $(".request-remove").click(function(event){
    ...
    $.ajax({
          type:'GET',
          url : '/Bit_Place/ajax/personal/deleteGetRequest.do?reqno='+reqno,
          success:function(response){
        	  $("#request-content").empty().append(response);
          }
          });
...
<c:if test="${data.accept == true}">
  <input type="hidden" id="isJoinedGroup" value="${data.accept}">
</c:if>
...
<div class="container">
   <div class="row">
      <div class="conversation-wrap col-lg-3">
        ...
        <c:forEach var="r" items="${data.object1}">
          <div class="media conversation" id="invitation-<%= idCount %>" data-inviterno="${r.reqno}">
                ...
                <h5 class="media-heading"><span class="join-requester">${r.name}</span>님이 팔로워 가입 요청</h5>
                    ...
                    <a class="request-remove request-remove glyphicon-tooltip" data-reqno="${r.reqno}" data-toggle="tooltip" data-placement="top" title="삭제" href="#"
                    id="request-remove-<%= idCount %>">
